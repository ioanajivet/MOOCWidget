...
public class DataCuration {
    ...
    static HashMap<String, UserForDataCuration> users = new HashMap<>();

    public static void cleanData2014() throws IOException, ParseException {
        readUsers(users, "data\\ri\\2014\\RI2014_graduates.csv");

        readQuizSessions("data\\ri\\2014\\quiz_sessions.csv");
        cleanQuizSessions();
        writeQuizSessions("data\\ri\\2014\\new_quiz_sessions.csv");

        readForumSessions("data\\ri\\2014\\forum_sessions.csv");
        cleanForumSessions();
        writeForumSessions("data\\ri\\2014\\new_forum_sessions.csv");

        readObservations("data\\ri\\2014\\observations.csv");
        cleanVideoSessions();
        writeVideoSessions("data\\ri\\2014\\new_video_sessions.csv");

        createActivitySessions();
        createArtificalSessions();
        writeActivitySessions("data\\ri\\2014\\activity_sessions.csv");
        writeArtificialSessions("data\\ri\\2014\\new_sessions.csv");


    }

    public static void cleanData(int week) throws IOException, ParseException {
        readUsers(users, "data\\ri\\2016\\RI2016_test.csv");

        readQuizSessions("data\\ri\\2016\\week" + week + "\\data\\quiz_sessions.csv");
        cleanQuizSessions();
        writeQuizSessions("data\\ri\\2016\\week" + week + "\\data\\curated\\new_quiz_sessions.csv");

        readForumSessions("data\\ri\\2016\\week" + week + "\\data\\forum_sessions.csv");
        cleanForumSessions();
        writeForumSessions("data\\ri\\2016\\week" + week + "\\data\\curated\\new_forum_sessions.csv");

        readObservations("data\\ri\\2016\\week" + week + "\\data\\observations.csv");
        cleanVideoSessions();
        writeVideoSessions("data\\ri\\2016\\week" + week + "\\data\\curated\\new_video_sessions.csv");

        createActivitySessions();
        createArtificalSessions();
        writeActivitySessions("data\\ri\\2016\\week" + week + "\\data\\curated\\activity_sessions.csv");
        writeArtificialSessions("data\\ri\\2016\\week" + week + "\\data\\curated\\new_sessions.csv");
    ...
    private static void readForumSessions(String filename) throws IOException, ParseException {
        ...
        while ((nextLine = csvReader.readNext()) != null) {
            ...
            if(users.containsKey(session_attr[3])) {
                ...
                users.get(session_attr[3]).addForumSession(new Session(session_attr[3], session_attr[4], session_attr[5], duration));
    ...
    private static void readQuizSessions(String filename) throws IOException, ParseException {
        ...
        while ((nextLine = csvReader.readNext()) != null) {
            ...
            if(users.containsKey(session_attr[4])) {
                ...
                users.get(session_attr[4]).addQuizSession(new Session(session_attr[4], session_attr[5], session_attr[6], duration));
    ...
    private static void readObservations(String filename) throws IOException, ParseException {
        ...
        while ((nextLine = csvReader.readNext()) != null) {
            ...
            if(users.containsKey(session_attr[1])) {
                ...
                users.get(session_attr[1]).addVideoSession(new Session(session_attr[1], nextLine[4], nextLine[5], duration));
